	...
	return gallery && gallery.length > 0 ? (
		<>
			...
			<div className='flex justify-center items-center gap-1 md:gap-2 lg:gap-3 p-1 pb-4'>
				<div
					className='text-lg md:text-xl lg:text-2xl box-border cursor-pointer select-none px-1 transition-all font-bold text-custom-secondary/60 hover:text-custom-secondary/80 active:text-custom-secondary'
					onClick={prevSlide}
				>
					&#10094;
				</div>
				<div className='flex items-center gap-1 md:gap-2 lg:gap-3'>
					{gallery.map((item, index) => {
						return (
							<span
								key={item.id}
								className={`inline transition-all h-3 border-[2px] border-custom-secondary/60 w-3 md:h-4 md:w-4 bg-custom-secondary/60 hover:bg-custom-secondary/80 active:bg-custom-secondary rounded-lg cursor-pointer ${
									index === slideIndex
